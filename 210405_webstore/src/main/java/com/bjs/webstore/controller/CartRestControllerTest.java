...
public class CartRestControllerTest {
	...
	public void update_return_200_session() throws Exception {
		...
		RequestBuilder req = post("/rest/cart").content(asJsonString(cart)).contentType(MediaType.APPLICATION_JSON)
				.session(session);
		...
		req = put("/rest/cart/" + session.getId()).content(asJsonString(cart)).contentType(MediaType.APPLICATION_JSON)
				.session(session);
		this.mockMvc.perform(req).andExpect(status().isOk());
		
		req = get("/rest/cart/" + session.getId());
		this.mockMvc.perform(req).andExpect(jsonPath("$.cartItems[0].product.productId").value("P1235"))
				.andExpect(jsonPath("$.cartItems[1].product.productId").value("P1236"));
	}

	// @formatter:off
	
	@Test
	public void create_method_should_return_200()
	throws Exception {
	// Arrange
	...
	@Test
	public void update_method_should_return_200()
	throws Exception {
	// Arrange
	CartDto cart = new CartDto("C123");
	this.mockMvc.perform(post("/rest/cart").content(asJsonString(cart))
			.contentType(MediaType.APPLICATION_JSON)
		.session(session)).andExpect(status().isCreated());
	// Act
	CartItemDto cartItemDto = new CartItemDto();
	...
	this.mockMvc.perform(put("/rest/cart/add/P1234")
			.session(session)).andExpect(status().is(200));
	// Act
	this.mockMvc.perform(put("/rest/cart/remove/P1234")
			.session(session)).andExpect(status().isOk());
	}
